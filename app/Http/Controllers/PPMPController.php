...
class PPMPController extends Controller
{
    ...
    public function po_approved_ppmp($branch_id)
    {
        ...
        $ppmpNewRequests = ProProManPlan::leftJoin('item_details', 'item_details.id', '=', 'pro_pro_man_plans.item_details_id')
            ->leftJoin('units', 'units.id', '=', 'item_details.unit_id')
            ->select($this->select_all_ppmp, 'item_details.description', 'units.uom', 'item_details.price_catalogue')
            ->where('year', '=', $user->ppmp_year)
            ->where('pro_pro_man_plans.is_draft', '=', '0')
            ->where('pro_pro_man_plans.branches_id', '=', $branch_id)
            ->where('pro_pro_man_plans.is_bo_approve', '=', 1)
            ->where('pro_pro_man_plans.is_delete', 0)
            ->where('pro_pro_man_plans.is_pr_approve', 1)
            ->get();
    ...
    public function ppmp_activity_log($branch_id)
    {
        ...
        $ppmpHistories = ProProManPlanHistory::with(['changes_record_by.profile', 'ppmp' => function ($builder) use ($branch_id) {
            $builder
                ->where('year', getPpmpYear())
                ->where('branches_id', $branch_id);
        }, 'ppmp.item_detail', 'ppmp.branch'])
            ->whereHas('ppmp', function ($builder) use ($branch_id) {
                $builder
                    ->where('year', getPpmpYear())
                    ->where('branches_id', $branch_id);
            })
    ...
    public function print_activity_log($branch_id)
    {
        $ppmpHistories = ProProManPlanHistory::with(['changes_record_by.profile', 'ppmp' => function ($builder) use ($branch_id) {
            $builder
                ->where('year', getPpmpYear())
        ...
        return view('bo-dashboard/print-ppmp-activity-log')->with('ppmp_histories', $ppmpHistories)->with('branch_id', $branch_id);
    }
