<x-dashboard-layout>
    <x-slot:title>
        ...
        <div class="row mb-3">
            <div class="col-12">
                <a class="submit-btn btn btn-link btn-sm float-end"
                    href="{{ route('ppmp-activity-log.show', ['branch_id' => Auth::user()->branches_id]) }}"><em class="bi bi-clock-history"></em>
                    Activity Logs</a>
                <a href="{{ route('dashboard.show') }}" type="button" class="btn btn-sm btn-secondary"><em class="bi bi-plus"></em> Add new item</a>
                @if(count($cart_items) > 0)
                <button type="button" class="submit-btn btn btn-sm btn-primary" onclick="submitCart()"><em class="bi bi-save2"></em> Submit</button>
                @endif
            </div>
        </div>
        <div class="table-responsive mb-3">
            <table class="table table-sm table-hover border-dark caption-top" id="ppmp-cart">
                <caption>Project Procurement Management Plan Cart <span class="badge text-bg-primary">Year <strong>{{ Auth::user()->ppmp_year
                            }}</strong></span></caption>
                <thead class="text-center">
                    <tr>
                        <th style="width: 20%;" rowspan="2" scope="col">Item Description</th>
                        <th rowspan="2" scope="col">Unit</th>
                        <th rowspan="2" scope="col">Estimated Budget</th>
                        <th style="width: 50%;" colspan="{{ count($ppmp_format) }}" scope="col">Schedule/Milestone of Activities</th>
                ...
                <tbody>
                    ...
                    <tr @if ($item->is_priority === 1) class="table-primary" @endif>
                        ...
                        <td>{{ $item->remarks }}</td>
                        <td>
                            <a href="{{ route('get-ppmp-record.show', ['ppmp_id' => $item->id]) }}" type="button"
                                class="btn btn-primary btn-sm m-1"><em class="bi bi-pencil-fill"></em></a>
                            <button type="button" onclick="deleteFromCart({{$item->id}})" class="btn btn-danger btn-sm m-1"><em
        ...
        <x-slot:additional_script>
            ...
            <script defer>
                ...
                if (confirmDelete) {
                    await axios.delete(`{{ route('delete-ppmp.perform') }}/${itemId}`)
                        .then(res => {
