<x-dashboard-layout>
    ...
    @php
        $breadcrumb = [
            ...
            ['name' => 'Item Position List'],
        ]
    @endphp

    <x-breadcrumb :breadcrumb="$breadcrumb" />
    <h1 class="h5 card-title"><span class="float-end small"># of records: <span class="badge text-bg-secondary">{{ count($positions) }}</span></span></h1>
    <div class="mb-4">
    ...
    <div class="table-responsive">
        <table class="table table-sm table-hover border-dark" id="position-table">
            ...
            <tbody>
                @foreach ($positions as $position)
                    <tr>
                        ...
                        <td class="text-center"><button class="btn btn-sm btn-success" onclick="openEdit({{ $position->id }})" @if($position->is_delete===1) disabled @endif><em class="bi bi-pencil-square"></em></button></td>
                        <td>{{ $position->description }} @if($position->is_delete===1) <span class="badge bg-secondary">Position Deleted</span> @else <button type="button" class="btn btn-sm btn-danger" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;" onclick="deleteRecord({{$position->id}})"><em class="bi bi-trash-fill"></em></button> @endif</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <x-slot:additional_script>
        ...
        @include('layout/datatable', ['tableId' => 'position-table'])
    </x-slot>
</x-dashboard-layout>