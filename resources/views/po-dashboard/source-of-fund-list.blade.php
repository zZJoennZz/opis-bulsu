<x-dashboard-layout>
    ...
    <div class="table-responsive">
        <table class="table table-sm border-dark" id="source-of-fund-table">
            <caption>Source of Fund List</caption>
            <thead>
                <tr>
                    <th class="text-start" style="width: 5%;"></th>
                    <th class="text-start" style="width: 5%;">Edit</th>
            ...
            <tbody>
                @foreach ($source_of_funds as $source_of_fund)
                    <tr>
                        ...
                        <td class="text-center"><button class="btn btn-sm btn-success" onclick="openEdit({{ $source_of_fund->id }})" @if($source_of_fund->is_delete===1) disabled @endif><em class="bi bi-pencil-square"></em></button></td>
                        <td>{{$source_of_fund->description}} @if($source_of_fund->is_delete===1) <span class="badge bg-secondary">Source of Funds Deleted</span> @else <button type="button" class="btn btn-sm btn-danger" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;" onclick="deleteRecord({{$source_of_fund->id}})"><em class="bi bi-trash-fill"></em></button> @endif</td>
                        <td>{{ $source_of_fund->source_of_fund }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <x-slot:additional_script>
        ...
        @include('layout/datatable', ['tableId' => 'source-of-fund-table'])
    </x-slot>
</x-dashboard-layout>
