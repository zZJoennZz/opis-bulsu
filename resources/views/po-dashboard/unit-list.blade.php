<x-dashboard-layout>
    ...
    <div class="table-responsive">
        <table class="table table-sm table-hover border-dark" id="unit-table">
            ...
            <tbody>
                @foreach ($units as $unit)
                    <tr>
                        ...
                        <td class="text-center"><button class="btn btn-sm btn-success" onclick="openEdit({{ $unit->id }})" @if($unit->is_delete===1) disabled @endif><em class="bi bi-pencil-square"></em></button></td>
                        <td>{{ $unit->uom }} @if($unit->is_delete===1) <span class="badge bg-secondary">Unit Deleted</span> @else <button type="button" class="btn btn-sm btn-danger" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;" onclick="deleteRecord({{$unit->id}})"><em class="bi bi-trash-fill"></em></button> @endif</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <x-slot:additional_script>
        ...
        @include('layout/datatable', ['tableId' => 'unit-table'])
    </x-slot>
</x-dashboard-layout>