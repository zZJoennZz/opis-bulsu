@include('layout/header', ['title' => 'Item Categories | OPIS - BulSU e-PROCUREMENT'])
...
<div class="modal fade" id="addNewCategory" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="{{ route('item-cat.add') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="staticBackdropLabel">Add Item Category</h1>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form onsubmit="return saveChanges(event)">
                @csrf
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="edit-cat-header-text">Edit Item Category</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="description" class="form-label">Category Name</label>
                        <input type="text" class="form-control" id="edit_description" name="description" placeholder="Category name">
                    </div>
                    <div>
                        <label for="description" class="form-label">Under of what group (for reports)</label>
    ...
    <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            ...
            <div class="pt-3">
                <div class="card">
                    <div class="card-body">
                        <h1 class="h5 card-title">Item Category List <span class="float-end small"># of records: <span class="badge text-bg-secondary">{{ count($item_categories) }}</span></span></h1>
                        <hr />
                        <div class="mb-4">
                                        ...
                                        <tr>
                                            <td>
                                                <div class="form-check w-100 d-flex align-items-center justify-content-center">
                                                    <input class="form-check-input category-checkbox" type="checkbox" value="{{ $category->id }}" id="category{{ $category->id }}" @if($category->is_delete===1) disabled @endif>
                                                </div>
                                            </td>
    ...
    async function deleteRecord(id = null){
        if (id === null) {
            ...
            if (confirmDeleteSingle) {
                await axios.delete(`{{ url('item-categories/single') }}/${id}`)
                    .then(res => {
                        window.location.reload();
                    })
                    .catch(err => {
