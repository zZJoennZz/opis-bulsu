    ...
    <div class="col-sm-12 col-md-6">
        <div class="my-2">
            <div class="border border-primary rounded p-3">
                <a href="{{ route('ppmp-activity-log.show', ['branch_id' => $branch->id]) }}" class="float-end btn btn-link btn-sm"><em
                        class="bi bi-clock-history"></em> History Logs</a>
                <div>
                ...
                <div class="row">
                    <div class="col-4">
                        ...
                        @endphp
                        <a href="{{ $isDisabled ? " #" : route('bo-new-ppmp-request.show', ['branch_id'=> $branch->id]) }}" class="@if($isDisabled)
                            border @else shadow @endif h-100 w-100
                            rounded-4 p-4 text-center d-flex align-items-center justify-content-center flex-column position-relative
                            text-decoration-none" style="cursor: pointer;">
                            <div class="mb-md-2 position-absolute top-0 {{ $isDisabled ? " bg-secondary" : "bg-danger" }} text-light p-2"
                                style="width: 50px; height: 50px; border-radius: 100%; margin-top: -1rem;">
                    ...
                    <div class="col-4">
                        ...
                        @endphp
                        <a href="{{ $isDisabled ? " #" : route('approved-ppmp-request.show', ['branch_id'=> $branch->id]) }}" class="@if($isDisabled)
                            border @else shadow @endif h-100
                            w-100 rounded-4 p-4 text-center d-flex align-items-center justify-content-center flex-column position-relative
                            text-decoration-none" style="cursor: pointer;">
                            <div class="mb-md-2 position-absolute top-0 {{ $isDisabled ? " bg-secondary" : "bg-primary" }} text-light p-2"
                                style="width: 50px; height: 50px; border-radius: 100%; margin-top: -1rem;">
                    ...
                    <div class="col-4">
                        @php
                        $isDisabled = true;
                        if (count($branch->ppmp->where('year', '<>',getPpmpYear())->where('is_bo_approve', 1)->where('is_pr_approve',
                            1)->where('is_delete', 0)->where('is_draft', 0)) === 0) {
                            $isDisabled = true;
                            } else {
                            $isDisabled = false;
                            }
                            @endphp
                            <a href="{{ $isDisabled ? " #" : route('approved-ppmp-request.show', ['branch_id'=> $branch->id]) }}"
                                class="@if($isDisabled)
                                border @else shadow @endif h-100
                                w-100 rounded-4 p-4 text-center d-flex align-items-center justify-content-center flex-column position-relative
                                text-decoration-none" style="cursor: pointer;">
...
<div class="row border-bottom rounded-5 p-3 mb-3">
    <div class="col-12 col-lg-6 text-start p-3">
        ...
        <div>
            <a href="{{ route('ppmp-activity-log.show', ['branch_id' => $ppmp->id]) }}" class="btn btn-secondary btn-sm"><em
                    class="bi bi-clock-history"></em> PPMP History Logs</a>
        </div>
    </div>
    <div class="col-12 col-lg-2 mb-4">
        ...
        @if ($count["count"] > 0)
        <a href="{{ route('bo-new-ppmp-request.show', ['branch_id' => $ppmp->id]) }}"
            class="ppmpCard shadow h-100 w-100 rounded-4 p-4 text-center d-flex align-items-center justify-content-center flex-column position-relative text-decoration-none"
        ...
        @else
        <a class="shadow h-100 w-100 rounded-4 p-4 text-center d-flex align-items-center justify-content-center flex-column position-relative text-decoration-none"
            style="cursor: pointer;">
    ...
    <div class="col-12 col-lg-2 mb-4">
        ...
        @if ($count["count"] > 0)
        <a href="{{ route('approved-ppmp-request.show', ['branch_id' => $ppmp->id]) }}"
            class="ppmpCard shadow h-100 w-100 rounded-4 p-4 text-center d-flex align-items-center justify-content-center flex-column position-relative text-decoration-none"
            style="cursor: pointer;">
            <div class="mb-md-2 position-absolute top-0 bg-primary text-light p-2" style="width: 50px; height: 50px; border-radius: 100%;">
                <em class="bi bi-journal-check" style="font-size: 1.4rem;"></em>
