<x-dashboard-layout>
    <x-slot:title>
        ...
        @php
        if ($tranItem->transaction->type !== "PAR") {
        $breadcrumb = [
        ...
        $breadcrumb = [
        ['name' => '<em class="bi bi-house-fill"></em>', 'route' => 'dashboard.show'],
        ['name' => 'All PAR Records', 'route' => 'par.all'],
        ['name' => 'Transfer PAR Item'],
        ];
        }
        @endphp
        @php
        $quote = $tranItem->bac_reso_item->quotation;
        @endphp
        <x-breadcrumb :breadcrumb="$breadcrumb" />
        <div class="card card-table d-inline float-end ms-2 mb-2">
            <div class="card-body">
                <h5 class="card-title">{{ $tranItem->bac_reso_item->quotation->pr_item->ppmp->item_detail->description }}</h5>
                <div class="card-text">
                    <table class="table table-bordered">
                        ...
                        <tr>
                            ...
                            <td>{{ count($tranItem->properties) }} {{ $tranItem->bac_reso_item->quotation->pr_item->ppmp->item_detail->unit->uom }}
                            </td>
                        </tr>
                        <tr>
                            <th>Unit Price</th>
        ...
        <form method="POST" action="{{ route('transfer_ics.perform') }}/{{ $tranItem->id }}" id="ics-transfer-form">
            @csrf
            <div class="row mt-2 mb-3">
                <div class="col-5 border py-2">
                    ...
                    <div>
                        <div>{{ $tranItem->transaction->receivers[0]->end_user->first_name }} {{
                            $tranItem->transaction->receivers[0]->end_user->middle_name }} {{
                            $tranItem->transaction->receivers[0]->end_user->last_name
                            }}
                        </div>
                        <div class="small text-muted">{{ $tranItem->transaction->receivers[0]->end_user->position->name }}</div>
                        <div class="small text-muted">{{ $tranItem->transaction->receivers[0]->end_user->branch->branch_name }}</div>
                    </div>
                    @else
                    <label for="issuer" class="text-uppercase small text-muted">Issuer</label>
                    <select class="form-select" id="issuer" name="issuer" aria-label="Default select issuer">
                        <option selected hidden disabled>Open to select issuer</option>
                        @foreach ($tranItem->transaction->receivers as $endUser)
                        <option value="{{ $endUser->end_user->id }}">{{ $endUser->end_user->first_name }} {{ $endUser->end_user->middle_name }} {{
                            $endUser->end_user->last_name }} ({{
                            $endUser->end_user->position->name }} / {{ $endUser->end_user->branch->branch_name }})</option>
                        @endforeach
                    </select>
                    @endif
                </div>
                ...
                <div class="col-5 border py-2">
                    ...
                    <select class="form-select" id="receiver" name="receiver" aria-label="Default select receiver">
                        ...
                        @if ($tranItem->transaction->receivers[0]->end_user->id !== $endUser->id)
            ...
            @if ($tranItem->properties[0]->serial_number !== "n/a")
            <div class="row mb-3">
                <div class="col-12">
                    <div class="form-label">Select Unit/s to Transfer</div>
                    @foreach ($tranItem->properties as $property)
            ...
            <div class="row mb-3">
                <div class="col-12">
                    ...
                    <input required type="number" class="form-control" id="quantity" name="quantity" min="1" max="{{ count($tranItem->properties) }}">
                </div>
            </div>
            @endif
            <div class="row mb-3">
